...
function OS_onPageEvent(e)
{
	console.log(e);
	
	var handler=OS_getPageEventHandler(e.target.id,e.type);

	if (handler)
    	handler(e);
    else
    	console.error('No handler for event: ',e.type);
}

function OS_getPageEventHandler(page_id,event_type)
{
	var page=page_id.replace('page','');
    var name=event_type.replace('page','');
    
    var handler=page+'_on'+name.charAt(0).toUpperCase()+name.substring(1);
...
function OS_onOrientationChanged(e)
{
	var page=$.mobile.activePage;
	var errorlabels=page.find('.os-ui-error-label');
	OS_updateErrorPosition(errorlabels);
}
...
function OS_focusNext(el)
{
	...
	console.log('And the focus goes to: '+best_match.id);
...
function OS_checkPrerequisites()
{
	//check for JSON
